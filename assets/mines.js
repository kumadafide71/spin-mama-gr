...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-95a3318f";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-95a3318f">'+'<span class="tile-face-95a3318f tile-front-95a3318f"></span>'+'<span class="tile-face-95a3318f tile-back-95a3318f"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-95a3318f")){return;}
var back=tile.querySelector(".tile-back-95a3318f");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-95a3318f");if(isMine){tile.classList.add("boom-95a3318f");}
if(dim){tile.classList.add("dimmed-95a3318f");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-95a3318f")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-95a3318f");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-95a3318f")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-95a3318f");void panelEl.offsetWidth;panelEl.classList.add("jolt-95a3318f");}
endRound();}
